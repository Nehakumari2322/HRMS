...
class Client{
    ...
    public function getLeadDetails($c_id){
        $this->db->query(' SELECT DISTINCT l.p_id, l.l_id, l.name, l.email, l.phone, l.status, l.source,l.assign_to,l.requirement,l.vendor_remarks, l.client_remarks,l.created_ts,l.updated_ts FROM lead l,project p,client c WHERE p.p_id = l.p_id AND p.c_id = :c_id ORDER BY l.created_ts DESC');
        $this->db->bind(':c_id', $c_id);
        $row = $this->db->resultSet();
        return $row;
    }
    ...
    public function getLeadDetailsWithStatus($status,$c_id){
        $this->db->query(' SELECT l.l_id,l.name,l.email,l.phone, l.status, l.source, l.vendor_remarks, l.client_remarks, l.created_ts, l.updated_ts,p.name as project_name,l.assign_to,l.requirement FROM lead l,project p WHERE l.p_id = p.p_id AND l.status = :status AND p.c_id = :c_id  ORDER BY l.created_ts DESC');
        $this->db->bind(':status',$status);
        $this->db->bind(':c_id',$c_id);
        $row = $this->db->resultSet();
        return $row; 
    ...
    public function getLeadDetailsWithProjectAndStatus($c_id,$project_id,$status){
        $this->db->query(' SELECT l.l_id,l.name,l.email,l.phone, l.status, l.source, l.vendor_remarks, l.client_remarks, l.created_ts, l.updated_ts,p.name as project_name,l.assign_to,l.requirement FROM lead l,project p WHERE l.p_id = p.p_id AND l.status =:status  AND p.c_id = :c_id AND p.p_id = :project_id ORDER BY l.created_ts DESC');
        $this->db->bind(':status',$status);
        $this->db->bind(':c_id',$c_id);
        $this->db->bind(':project_id',$project_id);
        $row = $this->db->resultSet();
